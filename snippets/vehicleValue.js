...
vt = await fetch('https://api.lss-manager.de/de_DE/vehicles').then(res =>
    res.json()
);
console.log(
    `${v.length.toLocaleString()} Fahrzeuge mit einem Gesamtwert von ${v
        .map(v => vt[v.vehicle_type].credits)
        .reduce((a, b) => a + b, 0)
